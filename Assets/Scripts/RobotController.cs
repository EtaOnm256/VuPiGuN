...
public class RobotController : Pausable
{
    ...
    bool jumpslash_as_lowerslash = false;
    ...
    private float _headaimweight = 0.0f;

    private float _chestaimweight = 0.0f;

    private float _rarmaimweight = 0.0f;
    ...
    private float _barmlayerweight = 0.0f;
            ...
            //if (lowerBodyState != LowerBodyState.DOWN || !Grounded)
            {
                if ((!Grounded && knockBackType!=KnockBackType.Aerial) || knockBackType == KnockBackType.Down || knockBackType == KnockBackType.KnockUp)
                {
                    ...
                    if (knockBackType == KnockBackType.KnockUp && lowerBodyState != LowerBodyState.DOWN)
                    {
                        ...
                        knockbackdir.y = 1.0f;
                        _speed = 0.0f;
                        _verticalVelocity = robotParameter.KnockbackSpeed*1.0f;
                        strongdown = true;
                    }
                    else
                    {
    ...
    private void Start()
    {
        ...
        if (robotParameter.itemFlag.HasFlag(ItemFlag.SteadyShoot))
        {
            shoot_skill_num++;
        }
    ...
    private void DeadBodyMove()
    {
        ...
        if (hitslow_timer <= 0 && hitstop_timer <= 0)
        {
            ...
            headmultiAimConstraint.weight = _headaimweight;
            ...
            chestmultiAimConstraint.weight = _chestaimweight;
            ...
            overrideTransform.weight = _rarmaimweight;

            animator.SetLayerWeight(2, _barmlayerweight);

            if (!dualwielding)
                animator.SetLayerWeight(1, _rarmaimweight);
    ...
    private void GroundedCheck()
    {
        ...
        switch (lowerBodyState)
        {
            ...
            case LowerBodyState.AIRHEAVYFIRE:
                ...
                break;
            case LowerBodyState.AIRSTEADYHEAVYFIRE:
                if (Grounded)
                {
                    TransitLowerBodyState(LowerBodyState.GROUND);
    ...
    private void UpperBodyMove()
    {
        ...
        float rhandaimweight_thisframe = 0.0f;
        ...
        switch (upperBodyState)
        {
            ...
            case UpperBodyState.STEADYFIRE:
                {
                    miragecloud_invalid = true;

                    if (upperBodyState == UpperBodyState.FIRE || upperBodyState == UpperBodyState.STEADYFIRE)
                    {
                        ...
                        if (dualwielding || upperBodyState == UpperBodyState.STEADYFIRE)
                        {
                            aiming_factor = animator.GetCurrentAnimatorStateInfo(2).normalizedTime;
                            rhandaimweight_thisframe = Mathf.Clamp((aiming_factor - 0.70f) * 4, 0.0f, 1.0f);
                            _barmlayerweight = Mathf.Min(1.0f, _barmlayerweight + 0.08f * firing_multiplier);
                    ...
                    if (!fire_done)
                    {
                        ...
                        {
                            shoot = event_rollingfired || event_fired;
                        }
                        else
                        {
                            shoot = event_fired;
                        ...
                        if (shoot)
                        {
                            ...
                            if (upperBodyState != UpperBodyState.STEADYFIRE)
                            {
                            ...
                            if (upperBodyState == UpperBodyState.ROLLINGFIRE)
                            {
                                ...
                                if (event_fired)
                                {
                                    ...
                                    else // lowerBodyState == LowerBodyState.AIRROLLINGFIRE
                                        TransitLowerBodyState(LowerBodyState.AIR);
                    ...
                    else
                    {
                        ...
                        if (fire_followthrough <= 0)
                        {
                            ...
                            else if (lowerBodyState == LowerBodyState.STEADYFIRE || lowerBodyState == LowerBodyState.AIRSTEADYFIRE)
                            {
                                if (Grounded)
                                    TransitLowerBodyState(LowerBodyState.STAND);
                                else
            ...
            case UpperBodyState.SUBFIRE:
                {
                    ...
                    {
                        _rarmaimweight = Mathf.Max(0.0f, _rarmaimweight - 0.04f);
                        _chestaimweight = Mathf.Max(0.0f, _chestaimweight - 0.04f);

                        //if (dualwielding)
                        _barmlayerweight = Mathf.Min(1.0f, _barmlayerweight + 0.08f);
                        //else
                        //    _barmlayerweight = Mathf.Max(0.0f, _barmlayerweight - 0.08f);
                        ...
                        if (Target_Robot != null)
                        {
                            ...
                            if (angle > 60)
                            {
                                _headaimweight = Mathf.Max(0.0f, _headaimweight - 0.1f);
                                head_no_aim_smooth = true;
                            }
                            else
                                _headaimweight = Mathf.Min(1.0f, _headaimweight + 0.1f);
                        }
                        else
                        {
                            _headaimweight = Mathf.Max(0.0f, _headaimweight - 0.1f);
                    ...
                    {
                        _chestaimweight = Mathf.Min(1.0f, _chestaimweight + 0.08f);
                        chest_pitch_aim = true;
                        _headaimweight = 0.0f;
                        _rarmaimweight = Mathf.Max(0.0f, _rarmaimweight - 0.16f);
                        _barmlayerweight = Mathf.Min(1.0f, _barmlayerweight + 0.08f);
                    }
            ...
            case UpperBodyState.STEADYHEAVYFIRE:
                {
                    miragecloud_invalid = true;
                    ...
                    if (!fire_done)
                    {
                        ...
                        if (shoot)
                        {
                            ...
                            if (upperBodyState == UpperBodyState.STEADYHEAVYFIRE)
                                fire_followthrough = 30;
                            else
                                fire_followthrough = rightWeapon.fire_followthrough;
                        }
                        ...
                        else if (upperBodyState == UpperBodyState.STEADYHEAVYFIRE)
                        {
                            aiming_factor = animator.GetCurrentAnimatorStateInfo(0).normalizedTime;
                            rhandaimweight_thisframe = Mathf.Clamp((aiming_factor - 0.0f) * 4.0f, 0.0f, 1.0f);
                        }
                        else
                        {
                            aiming_factor = animator.GetCurrentAnimatorStateInfo(2).normalizedTime;
                            rhandaimweight_thisframe = Mathf.Clamp((aiming_factor - 0.0f) * 4, 0.0f, 1.0f);
                        }
                    }
                    else
                    {
                        ...
                        if (fire_followthrough <= 0)
                        {
                            ...
                            if (lowerBodyState == LowerBodyState.STEADYHEAVYFIRE || lowerBodyState == LowerBodyState.AIRSTEADYHEAVYFIRE)
                            {
                                if (Grounded)
                                    TransitLowerBodyState(LowerBodyState.STAND);
                                else
                        ...
                        aiming_factor = rhandaimweight_thisframe = 1.0f;

                    }

                    if (upperBodyState == UpperBodyState.HEAVYFIRE)
                    {
                        _headaimweight = Mathf.Min(1.0f, _headaimweight + 0.10f);
                        _rarmaimweight = 0.0f;
            ...
            case UpperBodyState.VOIDSHIFT:
                ...
                else
                {
                    _chestaimweight = 0.0f;
                    _headaimweight = 0.0f;
                    _rarmaimweight = Mathf.Max(0.0f, _rarmaimweight - 0.08f);
                    _barmlayerweight = Mathf.Max(0.0f, _barmlayerweight - 0.08f);
                }
        ...
        rhandmultiAimConstraint.weight = rhandaimweight_thisframe;
    ...
    Quaternion Process_Aiming_Chest(bool aiming)
    {
        ...
        if (aiming)
        {
            ...
            if (rightWeapon == null || rightWeapon.trajectory != Weapon.Trajectory.Curved || (upperBodyState != UpperBodyState.HEAVYFIRE && upperBodyState != UpperBodyState.ROLLINGHEAVYFIRE && upperBodyState != UpperBodyState.STEADYHEAVYFIRE))
            {
                result = Quaternion.LookRotation(Target_Robot.GetTargetedPosition() - GetCenter(), new Vector3(0.0f, 1.0f, 0.0f));
            }
            else
            ...
            else if (upperBodyState == UpperBodyState.HEAVYFIRE || upperBodyState == UpperBodyState.ROLLINGHEAVYFIRE)
            {
                result = result * Quaternion.Euler(0.0f, 30.0f, 0.0f);
            }
        }
        else
        {
    ...
    Quaternion Process_Aiming_RHand(bool aiming)
    {
        ...
        if (aiming)
        {
            ...
            Quaternion oTR_q = Quaternion.Euler(overrideTransform.data.rotation);

            if (gatling)
            {
                overrideTransform.data.rotation = (oTR_q * Quaternion.Euler(0.0f, 0.0f, 20.0f)).eulerAngles;
            }
            else if (upperBodyState == UpperBodyState.HEAVYFIRE || upperBodyState == UpperBodyState.ROLLINGHEAVYFIRE)
            {
                overrideTransform.data.rotation = (oTR_q * Quaternion.Euler(0.0f, 0.0f, 30.0f)).eulerAngles;
            }
        }
        else
        {
    ...
    private void LowerBodyMove()
    {
        ...
        if (hitstop_timer <= 0)
        {
            ...
            switch (lowerBodyState)
            {
                ...
                case LowerBodyState.STEADYFIRE:
                case LowerBodyState.AIRSTEADYFIRE:
                case LowerBodyState.STEADYHEAVYFIRE:
                case LowerBodyState.AIRSTEADYHEAVYFIRE:
                    {
                        // a reference to the players current horizontal velocity
                        float currentHorizontalSpeed = new Vector3(_controller.velocity.x, 0.0f, _controller.velocity.z).magnitude;
                        ...
                        if (!backblast_processed &&
                            ...
                            || lowerBodyState == LowerBodyState.STEADYHEAVYFIRE)))
                        {
                            if (lowerBodyState == LowerBodyState.AIRHEAVYFIRE || lowerBodyState == LowerBodyState.HEAVYFIRE
                                || lowerBodyState == LowerBodyState.AIRSTEADYHEAVYFIRE || lowerBodyState == LowerBodyState.STEADYHEAVYFIRE)
                            {
                                ...
                                if (lowerBodyState == LowerBodyState.AIRHEAVYFIRE || lowerBodyState == LowerBodyState.AIRSTEADYHEAVYFIRE)
                                {
                        ...
                        else
                        {
                            ...
                            else if (lowerBodyState == LowerBodyState.STEADYHEAVYFIRE)
                            {
                                brakefactor = 0.5f;
                            }
                            ...
                            if (robotParameter.itemFlag.HasFlag(ItemFlag.FlightUnit) &&
                                !(lowerBodyState == LowerBodyState.AIRSTEADYFIRE || lowerBodyState == LowerBodyState.AIRSTEADYHEAVYFIRE
                                || lowerBodyState == LowerBodyState.STEADYFIRE || lowerBodyState == LowerBodyState.STEADYHEAVYFIRE))
                            {
                                if (_input.jump)
                                {
                                    if (ConsumeBoost(4))
                        ...
                        if (Target_Robot != null)
                        {
                            ...
                            // ローリングショット時は、平行移動としてさらに関数末尾で処理してる
                            if (lowerBodyState == LowerBodyState.ROLLINGFIRE || lowerBodyState == LowerBodyState.AIRROLLINGFIRE
                                || lowerBodyState == LowerBodyState.ROLLINGHEAVYFIRE || lowerBodyState == LowerBodyState.AIRROLLINGHEAVYFIRE)
                            {
                                ...
                                if (lowerBodyState == LowerBodyState.ROLLINGHEAVYFIRE || lowerBodyState == LowerBodyState.AIRROLLINGHEAVYFIRE
                                    || dualwielding)
                                {
                                    rotation_pitch = 0.0f;
                                }
                                ...
                                {
                                    rotation_pitch = Mathf.Clamp(Mathf.DeltaAngle(0.0f, look.eulerAngles.x), -seedSlash_PitchLimit, seedSlash_PitchLimit);
                                }
                        ...
                        else if(lowerBodyState == LowerBodyState.AIRSTEADYFIRE || lowerBodyState == LowerBodyState.AIRSTEADYHEAVYFIRE)
                        {
                            _verticalVelocity = 0.0f;
                            boosting = true;
                            ConsumeBoost(2);
                ...
                case LowerBodyState.VOIDSHIFT:
                    {
                        ...
                        else
                        {
                            ...
                            if(lowerBodyState == LowerBodyState.SLASH_DASH && (subState_Slash == SubState_Slash.AirSlashSeed || subState_Slash == SubState_Slash.SlideSlashSeed)
                                && lowerslash && Sword.haslowerslash)
                            {
                                if (Target_Robot != null)
                                {
                                    Vector3 target_dir = Target_Robot.GetTargetedPosition() - GetCenter();
                ...
                case LowerBodyState.JumpSlash:
                    {
                        ...
                        {
                            if(jumpslash_as_lowerslash)
                                _speed = targetSpeed = Sword.motionProperty[SubState_Slash.AirSlashSeed].DashSpeed;
                            else
        ...
        if (!hitslow_now && !hitstop_now)
        {
            ...
            else if ( (lowerBodyState == LowerBodyState.SLASH_DASH || lowerBodyState == LowerBodyState.VOIDSHIFT) && subState_Slash == SubState_Slash.DashSlash) // Sword.dashslash_cutthrough有効時のDASHSLASH_DASH
            {
                ...
                || lowerBodyState == LowerBodyState.HEAVYFIRE || lowerBodyState == LowerBodyState.STEADYHEAVYFIRE))
                ||
                ...
                )
            {
                ...
                if ((lowerBodyState == LowerBodyState.HEAVYFIRE || lowerBodyState == LowerBodyState.ROLLINGHEAVYFIRE || lowerBodyState == LowerBodyState.STEADYHEAVYFIRE) && !ground_boost_now)
                {
                    if (!backblast_processed)
                    {
                        Vector3 backBlastDir = -(rightWeapon.gameObject.transform.rotation * (Vector3.forward));
            ...
            {
                if (lowerBodyState == LowerBodyState.AIRHEAVYFIRE || lowerBodyState == LowerBodyState.HEAVYFIRE || lowerBodyState == LowerBodyState.AIRSTEADYHEAVYFIRE || lowerBodyState == LowerBodyState.STEADYHEAVYFIRE)
                {
            ...
            if (robotParameter.itemFlag.HasFlag(ItemFlag.SteadyShoot))
            {
                ...
                if (fire_dispatch && (ringMenuDir == RingMenuDir.Down) && ConsumeShoot())
                {
                    if (rightWeapon.heavy)
                    {
                        if (!Grounded)
                        {
                            lowerBodyState = LowerBodyState.AIRSTEADYHEAVYFIRE;
                        }
                        else
                        {
                            lowerBodyState = LowerBodyState.STEADYHEAVYFIRE;
                        ...
                        upperBodyState = UpperBodyState.STEADYHEAVYFIRE;
                    ...
                    else
                    {
                        ...
                        if (!Grounded)
                        {
                            lowerBodyState = LowerBodyState.AIRSTEADYFIRE;
                        }
                        else
                        {
                            lowerBodyState = LowerBodyState.STEADYFIRE;
                        }

                        upperBodyState = UpperBodyState.STEADYFIRE;
                        ...
                        else
                        {
                            ...
                            animator.Play(_animIDSnipeFire, 2, 0.0f);
                        }
