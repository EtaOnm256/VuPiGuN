...
half3 MixFogCube(real3 fragColor, real fogFactor,float3 viewDir)
{
   
    half4 tex = texCUBE(_SkyCubemap, viewDir);
        half3 c = DecodeHDR(tex, _SkyCubemap_HDR);
        c = c * _Tint.rgb * unity_ColorSpaceDouble.rgb;
        c *= _Exposure;

    return MixFogColor(fragColor, c, fogFactor);
}

void SplatmapFinalColor(inout half4 color, half fogCoord, float3 viewDir)
{
    color.rgb *= color.a;
    ...
    #ifdef TERRAIN_SPLAT_ADDPASS
        color.rgb = MixFogCube(color.rgb, half3(0,0,0), fogCoord, viewDir);
    #else
        color.rgb = MixFogCube(color.rgb, fogCoord, viewDir);
    #endif
...
Varyings SplatmapVert(Attributes v)
{
    ...
    o.viewDir = -GetWorldSpaceNormalizeViewDir(Attributes.positionWS);
...
#endif
{
    ...
    SplatmapFinalColor(color, inputData.fogCoord,IN.viewDir);
